"""
scripts/update_data.py  —  v29 (Override protection for DE + BE + LU + FR + ES + PT + GB + IE + IS + NO + SE + FI)
...
def http_get(url, timeout=30):
    req = urllib.request.Request(url, headers={"User-Agent": "EV-Map-Bot/29.0"})
...
def write_files(monthly, annual):
    ...
    fi_override = load_override("finland_monthly_override.json")   # ← neu für Finnland
    ...
    for ecb_code, (name, estat_code, pop) in COUNTRIES.items():
        ...
        elif ecb_code == "FI" and fi_override:                    # ← neu
            m = fi_override
            print("[Override] Finland monthly data protected")
...
def main():
    ...
    print("\n✓ Done – Overrides for DE, BE, LU, FR, ES, PT, GB, IE, IS, NO, SE + FI active (data up to March 2026)")
