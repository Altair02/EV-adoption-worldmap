"""
scripts/update_data.py  —  v20 (Override-Schutz für Deutschland)
==============================
- Deutschland: monatliche Daten werden aus germany_monthly_override.json geschützt
- Niederlande: RDW-Daten (wie bisher)
- Alle anderen Länder: normales ECB + Eurostat Verhalten
"""
...
# ── Override für Deutschland (geschützt) ─────────────────────
def load_germany_override():
    ...
    if override_path.exists():
        try:
            ...
            print(f"[Override] germany_monthly_override.json geladen ({len(data.get('monthly', {}).get('labels', []))} Monate)")
            return data.get("monthly")
        except Exception as e:
            print(f"[Override] Fehler beim Laden: {e}")
    else:
        print("[Override] germany_monthly_override.json nicht gefunden!")
    return None
...
def write_files(monthly, annual):
    ...
    for ecb_code, (name, estat_code, pop) in COUNTRIES.items():
        ...
        if ecb_code == "DE" and germany_override:
            m = germany_override
            print(f"[Override] Deutschland monatliche Daten aus Override-Datei übernommen ({len(m.get('labels', []))} Monate)")
...
def main():
    ...
    print(f"\n✓ Fertig — Deutschland monatliche Daten sind jetzt dauerhaft geschützt!")
