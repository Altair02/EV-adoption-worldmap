...
- Belgien:     Override (belgium_monthly_override.json) → FEBIAC
- Niederlande: RDW
"""
...
def http_get(url, timeout=30):
    req = urllib.request.Request(
        url,
        headers={"User-Agent": "EV-Map-Bot/21.0 (github.com/Altair02/EV-adoption-worldmap)"}
    )
...
def load_germany_override():
    path = DATA_DIR / "germany_monthly_override.json"
    if path.exists():
        try:
            data = json.loads(path.read_text("utf-8"))
            print(f"[Override] Germany geladen ({len(data.get('monthly', {}).get('labels', []))} Monate)")
...
def load_belgium_override():
    path = DATA_DIR / "belgium_monthly_override.json"
    if path.exists():
        try:
            data = json.loads(path.read_text("utf-8"))
            print(f"[Override] Belgium geladen ({len(data.get('monthly', {}).get('labels', []))} Monate)")
            return data.get("monthly")
        except Exception as e:
...
def fetch_rdw_netherlands():
    print("[RDW] Lade monatliche Daten für Niederlande...")
    rdw_data = {
    ...
    print(f"[RDW] Niederlande: {len(labels)} Monate")
    return {"labels": labels, "total": totals}

# ── Schreiben mit Override ─────────────────────
def write_files(monthly, annual):
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    changed = []
    de_override = load_germany_override()
    be_override = load_belgium_override()
...
def send_telegram(changed, n_countries, latest_month):
    ...
    data = json.dumps({"chat_id": TELEGRAM_CHAT, "text": body, "parse_mode": "Markdown", "disable_web_page_preview": False}).encode("utf-8")
...
def main():
    ...
    print(f"\n✓ Fertig — Belgien jetzt mit Override-Schutz!")
